...
This example demonstrates an end-to-end publisher (Jetstream) → Drava (consumer) → app inference workflow using a pre-trained KNN model.
A one-row Iris feature vector is published through JetStream, delivered by Drava, decoded by
the app which then classify it using a pre-trained KNN model.
