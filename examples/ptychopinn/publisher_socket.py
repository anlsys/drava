"""PtychoPINN socket publisher.

Group-by-group publisher for the Drava socket transport (FIFO -> socat -> UNIX
socket). Wire format per frame: [4-byte big-endian length][raw frame bytes].
All generic plumbing (pacing, EOS, metrics) lives in ``drava_common``.
...
from publisher_util import load_publish_config, make_payload_generator  # noqa: E402
