#!/usr/bin/env python3
"""
SC Experiment 3: PtychoNN Stage 1 / Stage 2 thread scaling.

Sweep Stage 1 and Stage 2 worker counts while keeping batching fixed.  Results
come directly from benchmark_two_stages.py summary.csv; this script does not
import experiments/_common.py.
"""
...
def run_benchmark(args, out_dir: Path, stage1_threads: int, stage2_threads: int) -> Path:
    ...
    subdir = (
        out_dir
        / f"s1t{stage1_threads}_s2t{stage2_threads}"
        f"_ib{args.infer_batch}_cb{args.stage1_callback_batch}_s2cb{stage2_callback_batch}"
    )
...
def parse_args():
    ...
    p.add_argument("--stage1-threads-list", default="1,2,4,8")
    p.add_argument("--stage2-threads-list", default="1,2,4,8")
...
def main():
    args = parse_args()
    out_dir = make_run_dir("sc3_ptychonn_thread_scaling")
    rows: list[dict] = []
    for stage1_threads in parse_ints(args.stage1_threads_list):
        for stage2_threads in parse_ints(args.stage2_threads_list):
            bench_dir = run_benchmark(args, out_dir, stage1_threads, stage2_threads)
            for srow in read_rows(bench_dir / "summary.csv"):
                rows.append(
                    row_from_summary(args, stage1_threads, stage2_threads, bench_dir, srow)
                )
    ...
    out_csv = out_dir / "sc3_thread_scaling_summary.csv"
    write_rows(out_csv, rows, columns)
    print(f"[sc3-threads] wrote {len(rows)} rows -> {out_csv}")
