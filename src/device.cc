...
class socketbuf : public std::streambuf
{
    ...
    public:
        ...
        explicit socketbuf(int fd) : sockfd(fd)
        {
...
drava_device_main(
    ...
    if (thread->tid == 0)
    {
        ...
        if (sockfd < 0)
            LOGGER_FATAL("Could not open socket %s: %s", SOCK_PATH, strerror(errno));
        ...
        if (connect(sockfd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) == -1)
            LOGGER_FATAL("Could not connect socket %s: %s", SOCK_PATH, strerror(errno));
        ...
        while (std::getline(sockstream, line))
        {
            ...
            drava->runtime.team_task_spawn(team, [=] (task_t * task) {
                    // TODO: parse the line
                    LOGGER_DEBUG("Processing %s", line.c_str());
                }
            );
        }
        close(sockfd);
